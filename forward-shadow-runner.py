...
pipeline, with a current-period read-only API fallback, then executes the isolated
continuity engine unchanged.
"""
...
def monthly_funding(start,end):
    ...
    for month in months:
        ms=str(month)
        url=f'{MONTHLY}/BTCUSDT/BTCUSDT-fundingRate-{ms}.zip'
...
def api_funding(start,end):
    try:
        r=requests.get(FAPI,params={'symbol':'BTCUSDT','startTime':int(start.timestamp()*1000),'endTime':int(end.timestamp()*1000),'limit':1000},timeout=30)
...
def corrected_derivatives_recent(days=45):
    ...
    # The monthly archive may not yet contain the current month.
    newest=max((f.index.max() for f in fs if not f.empty),default=None)
    api_start=(newest+pd.Timedelta(seconds=1)) if newest is not None else start
    fs.extend(api_funding(api_start,end))
    ms=[]
    for day in pd.date_range(start.normalize(),end.normalize(),freq='D',tz='UTC'):
        ...
        if tc and oi and tak:
            ms.append(pd.DataFrame({'Date':pd.to_datetime(m[tc],utc=True,errors='coerce'),'open_interest':pd.to_numeric(m[oi],errors='coerce'),'taker_ls':pd.to_numeric(m[tak],errors='coerce')}).dropna().set_index('Date'))
    ...
    f=pd.concat(fs).sort_index(); f=f[~f.index.duplicated(keep='last')]
    m=pd.concat(ms).sort_index(); m=m[~m.index.duplicated(keep='last')]
    return f,m
