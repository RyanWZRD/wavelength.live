(() => {
  ...
  async function load(){
    try {
      ...
      const statusText = ready ? 'READY — MANUAL DEMO ONLY' : 'BLOCKED — DO NOT PLACE';
      root.innerHTML = `
        ...
        <div class="landing-card" style="border-color:${ready?'#496d52':'#66552a'}">
          <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-bottom:15px">
            <div>
              <div style="display:inline-flex;align-items:center;padding:7px 10px;border:1px solid ${ready?'#496d52':'#66552a'};border-radius:7px;background:${ready?'rgba(73,109,82,.14)':'rgba(102,85,42,.14)'};font:800 9px 'IBM Plex Mono',monospace;letter-spacing:.06em">${esc(statusText)}</div>
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:9px">
            ${[['Entry',fmt(t.entry_reference,2)],['Stop',fmt(t.stop,2)],['Target',fmt(t.target,2)],['MT4 size',fmt(t.valid_mt4_size,2)],['Est. risk',`£${fmt(t.estimated_risk_gbp,2)}`],['Actual risk',`${fmt(t.estimated_risk_pct,5)}%`]].map(([a,b])=>`<div style="border:1px solid #25364a;border-radius:8px;padding:12px;min-width:0"><div style="color:#8094a8;font:600 8px 'IBM Plex Mono',monospace;text-transform:uppercase">${a}</div><div style="margin-top:5px;font:800 18px 'Bricolage Grotesque',sans-serif;white-space:nowrap;overflow:visible">${esc(b)}</div></div>`).join('')}
          </div>
          <div style="margin-top:14px;padding:10px 12px;border:1px solid #344a61;border-radius:8px;color:#aebfd0;font:700 9px 'IBM Plex Mono',monospace;line-height:1.7">
            MANUAL DEMO ONLY · Confirm IG-DEMO and ${esc(t.symbol)} before any manual entry · Do not increase size above ${fmt(t.valid_mt4_size,2)} · No broker connection · No automatic execution · No live-money authority.
          </div>
          <div style="margin-top:10px;color:#879bae;font:500 9px 'IBM Plex Mono',monospace;line-height:1.6">Updated ${when(p.generated_at)}</div>
        </div>`;
    } catch (err) {
