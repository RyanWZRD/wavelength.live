(() => {
  ...
  async function load(){
    try {
      ...
      const signalBlocked = hasSignal && !ready;
      const statusText = ready
        ? 'READY — MANUAL DEMO ONLY'
        : noSignal
          ? 'NO SIGNAL — WAITING'
          : expired
            ? 'SIGNAL FOUND — EXPIRED'
            : staleData
              ? 'SIGNAL FOUND — STALE DATA'
              : 'SIGNAL FOUND — BLOCKED';
      const reason = noSignal
        ? 'No qualifying Gold setup. A broker execution quote is not required until a valid LONG or SHORT signal is present.'
        : (v.reason || (ready ? 'Ticket is within its permitted manual-entry window.' : 'A Gold signal was detected, but this ticket is not currently valid for manual entry.'));
      ...
      const reasonColor = ready || noSignal ? '#aebfd0' : '#e2b1aa';
      root.innerHTML = `
        ...
        <div class="landing-card" style="border-color:${border}">
          <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;margin-bottom:15px">
            <div>
              <div style="display:inline-flex;align-items:center;padding:7px 10px;border:1px solid ${border};border-radius:7px;background:${statusBg};font:800 9px 'IBM Plex Mono',monospace;letter-spacing:.06em">${esc(statusText)}</div>
          ...
          <div style="margin-bottom:12px;padding:10px 12px;border:1px solid ${border};border-radius:8px;color:${reasonColor};font:700 9px 'IBM Plex Mono',monospace;line-height:1.65">${esc(reason)}</div>
          ...
          ${ready
            ? `<div style="margin-top:14px;padding:10px 12px;border:1px solid #344a61;border-radius:8px;color:#aebfd0;font:700 9px 'IBM Plex Mono',monospace;line-height:1.7">MANUAL DEMO ONLY · Confirm IG-DEMO and ${esc(t.symbol)} before any manual entry · Do not increase volume above ${fmt(t.valid_mt4_size,2)} · No automatic execution · No live-money authority.</div>`
            : noSignal
              ? `<div style="margin-top:14px;padding:10px 12px;border:1px solid #344a61;border-radius:8px;color:#aebfd0;font:800 9px 'IBM Plex Mono',monospace;line-height:1.7">WAITING FOR A QUALIFYING GOLD SETUP · No manual action is required.</div>`
              : `<div style="margin-top:14px;padding:10px 12px;border:1px solid #7a4a43;border-radius:8px;color:#e2b1aa;font:800 9px 'IBM Plex Mono',monospace;line-height:1.7">SIGNAL DETECTED BUT NOT ENTRY-READY · DO NOT PLACE · Wait for a freshly generated CURRENT ticket.</div>`}
          <div style="margin-top:10px;color:#879bae;font:500 9px 'IBM Plex Mono',monospace;line-height:1.6">Ticket updated ${when(p.generated_at)}${!noSignal && v.signal_time?` · Signal ${when(v.signal_time)}`:''}${!noSignal && v.signal_age_minutes!=null?` · Age ${fmt(v.signal_age_minutes,1)} min`:''}</div>
        </div>`;
    } catch (err) {
