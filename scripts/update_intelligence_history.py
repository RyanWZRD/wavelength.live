...
def fetch_symbol(symbol,btc30):
    ...
    a30=perf(kd,30);d90=perf(kd,90);vs30=(a30-btc30) if a30 is not None and btc30 is not None else 0.0;funding=oi=None
    try:
        p=get(f'https://fapi.binance.com/fapi/v1/premiumIndex?symbol={pair}');o=get(f'https://fapi.binance.com/futures/data/openInterestHist?symbol={pair}&period=4h&limit=7');funding=float(p.get('lastFundingRate') or 0)
...
def main():
    ...
    for s in universe:
        try:
            snap=fetch_symbol(s,btc30);arr=data.setdefault('symbols',{}).setdefault(s,[]);arr.append(snap);data['symbols'][s]=arr[-KEEP:];updated+=1;print(s,snap['score'],snap['trend']);time.sleep(.08)
