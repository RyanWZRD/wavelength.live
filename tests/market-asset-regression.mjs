...
await page.waitForSelector('#decisionEvidence');await page.waitForSelector('#decisionPriority');assert.doesNotMatch(await page.locator('#decisionEvidence').innerText(),/Waiting for market evidence/i);assert.match(await page.locator('#decisionEvidenceBoundary').innerText(),/RESEARCH PRIORITISATION ONLY/);
const socketURLs=await page.evaluate(()=>window.__wlFakeSocketURLs||[]);assert.ok(socketURLs.some(u=>/btcusdt@ticker\/btcusdt@kline_4h/.test(u)),'stable Asset ticker+kline websocket missing');assert.ok(socketURLs.some(u=>/btcusdt@aggTrade\/btcusdt@kline_4h/.test(u)),'analysis chart aggTrade+kline websocket missing');
