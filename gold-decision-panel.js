...
(() => {
  const gold = document.getElementById('goldDecisionPanel');
  if (!gold) return;
  let root=document.getElementById('igMt4ManualDemoTicketPanel');
  if(!root){
    root=document.createElement('section');
    root.id='igMt4ManualDemoTicketPanel';
    root.className='card';
    root.innerHTML='<div class="card-head"><div><div class="card-title">IG MT4 Manual Demo Ticket</div><div class="card-meta">Loading read-only operator ticket…</div></div><span class="badge warn">LOADING</span></div>';
    gold.insertAdjacentElement('afterend',root);
  }
  ...
  async function loadTicket(){
    try{
      const r=await fetch(`ig-mt4-manual-demo-ticket.json?ts=${Date.now()}`,{cache:'no-store'}); if(!r.ok)throw new Error(`HTTP ${r.status}`);
      const p=await r.json(),t=p.ticket||{},ready=p.status==='PASS_MANUAL_DEMO_READY';
      root.innerHTML=`<div class="card-head"><div><div class="card-title">IG MT4 Manual Demo Ticket</div><div class="card-meta">Operator-facing Gold ticket · manual demo only · no automatic execution</div></div><span class="badge ${ready?'good':'warn'}">${ready?'MANUAL DEMO READY':'BLOCKED'}</span></div>
      <div class="grid4" style="margin-bottom:14px">${metric('Direction',t.direction||'—',`Quality ${t.entry_quality||'—'}`)}${metric('Symbol',t.symbol||'—',t.internal_symbol||'')}${metric('MT4 size',fmt(t.valid_mt4_size,1),`Target risk ${fmt(t.target_risk_pct,2)}%`)}${metric('Estimated risk',`£${fmt(t.estimated_risk_gbp,2)}`,`${fmt(t.estimated_risk_pct,5)}%`)}</div>
      <div class="grid4" style="margin-bottom:14px">${metric('Entry reference',fmt(t.entry_reference,4))}${metric('Stop',fmt(t.stop,4),`Distance ${fmt(t.stop_distance,4)}`)}${metric('Target',fmt(t.target,4))}${metric('Action',ready?'MANUAL DEMO ONLY':'DO NOT PLACE',ready?'Confirm IG-DEMO + symbol first':'Ticket prerequisites not satisfied')}</div>
    ...
    }catch(err){root.innerHTML=`<div class="card-head"><div><div class="card-title">IG MT4 Manual Demo Ticket</div><div class="card-meta">Read-only operator ticket</div></div><span class="badge warn">UNAVAILABLE</span></div><div class="empty">Manual demo ticket feed unavailable: ${esc(err.message)}</div>`;}
  }
  loadTicket(); setInterval(loadTicket,30000);
})();
