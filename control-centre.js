...
const badge=(s)=>{const x=String(s||'UNKNOWN').toUpperCase();const c=/PASS|FRESH|READY|VALIDATED|OK|BROAD|BULLISH|HEALTHY/.test(x)?'ok':/FAIL|STALE|MISSING|ERROR|HARD|ATTENTION/.test(x)?'bad':'warn';return `<span class="badge ${c}">${esc(x)}</span>`};
